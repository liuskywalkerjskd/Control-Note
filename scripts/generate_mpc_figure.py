#!/usr/bin/env python3
"""
Generate MPC Receding Horizon figure illustrating the "plan ahead, execute one step, replan"
concept on a double-integrator (position tracking, sinusoidal reference).
"""
import numpy as np
import matplotlib
matplotlib.use('Agg')
...
titles = ["Step $k=0$", "Step $k=5$", "Step $k=10$"]
pred_colors = ['#2ca02c', '#ff7f0e', '#9467bd']   # green, orange, purple

# ── plot ───────────────────────────────────────────────────────────────────────
fig, axes = plt.subplots(1, 3, sharey=True)
fig.suptitle("MPC Receding Horizon: Plan Ahead, Execute One Step, Replan",
...
for col, (k_now, title, pcolor) in enumerate(zip(snapshot_steps, titles, pred_colors)):
    ...
    ax.plot(pred_times, pred_pos, '-o', color=pcolor, ms=5,
            lw=2, label=f'Predicted (N={N})', zorder=4)
    ...
    ax.plot(k_now, pred_pos[0], 'o', color=pcolor, ms=12,
            mec='red', mew=2.5, zorder=6, label='Apply $u_0$')
    ...
    # ── region labels ──────────────────────────────────────────────────────────
    if k_now > 0:
        ax.text((show_start + k_now) / 2, 3.7, 'past',
                ha='center', va='bottom', fontsize=9,
                color='steelblue', fontstyle='italic')
    ax.text(k_now + N / 2, 3.7, f'prediction horizon\n$(N={N})$',
            ha='center', va='bottom', fontsize=9,
    ...
    if col == 0:
        legend_elements = [
            Line2D([0], [0], ls='--', color='black',  lw=1.5, label='Reference'),
            Line2D([0], [0], ls='-',  color='blue',   lw=2.5, label='Actual (past)'),
            Line2D([0], [0], ls='-',  color=pcolor,   lw=2,   label=f'Predicted (N={N})'),
            Line2D([0], [0], marker='o', color='w', markerfacecolor=pcolor,
                   markersize=10, mec='red', mew=2.5,           label='Apply $u_0$'),
        ]
