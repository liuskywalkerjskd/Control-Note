#!/usr/bin/env python3
"""
Generate DC motor step response figure: full 2nd-order model vs 1st-order (L≈0) approximation.
"""
import numpy as np
import matplotlib
matplotlib.use('Agg')
...
fig.suptitle("DC Motor Step Response: Full vs Simplified Model", fontsize=13)

# ---- Left subplot: Full 2nd-order model ----
color_rpm  = '#1f77b4'   # blue
color_curr = '#d62728'   # red

ax1.set_title("Full Model (2nd Order)", fontsize=11)
ax1.plot(t_ms, rpm_full, color=color_rpm, label='Speed (RPM)')
ax1.set_xlabel("Time (ms)")
ax1.set_ylabel("Speed (RPM)", color=color_rpm)
ax1.tick_params(axis='y', labelcolor=color_rpm)

ax1b = ax1.twinx()
ax1b.plot(t_ms, i_full, color=color_curr, linewidth=1.5, linestyle='--', label='Current (A)')
ax1b.set_ylabel("Current (A)", color=color_curr)
...
ax2.set_title("Simplified Model (L ≈ 0)", fontsize=11)
ax2.plot(t_ms, rpm_approx, color='#2ca02c', label='Speed — simplified')
ax2.plot(t_ms, rpm_full,   color=color_rpm, linestyle='--', linewidth=1.5,
         label='Speed — full (ref)')
