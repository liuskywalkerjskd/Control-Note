#!/usr/bin/env python3
"""
Generate a figure comparing ZOH vs Tustin (bilinear) discretization methods
...
fig.suptitle(r'Discretization Methods: ZOH vs Tustin'
...
ax1.plot(t_c, y_c, 'k--', lw=1.5, label='Continuous', zorder=5)
...
ax2.semilogx(w_cont,  mag_c,      'k',   lw=2,   label='Continuous')
ax2.semilogx(w_hz,    mag_zoh,    'C0',  lw=2,   label='ZOH')
ax2.semilogx(w_hz,    mag_tustin, 'C1',  lw=2,   label='Tustin')
ax2.semilogx(w_hz,    mag_prewarp,'g--', lw=1.5, label='Tustin (pre-warp)')

ax2.axvline(w_nyq, color='gray', ls=':', lw=1.2, label=f'Nyquist ({w_nyq:.0f} rad/s)')
...
ax3.plot(np.cos(theta), np.sin(theta), 'k-', lw=0.8, alpha=0.5, label='Unit circle')
...
ax3.set_title('Pole-Zero Map (z-plane)')
