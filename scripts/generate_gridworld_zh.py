#!/usr/bin/env python3
"""
Generate grid-world DP illustration for the Bellman equation section (Chinese version).
...
for ax_idx, (ax, title, show_cost, show_V, show_path) in enumerate(zip(
    ...
    if show_path:
        ...
        # Labels
        ax.text(0, -0.42, '起点', ha='center', va='bottom', fontsize=7,
               fontweight='bold', color='#1565C0')
        ax.text(n-1, n-1+0.42, '目标', ha='center', va='top', fontsize=7,
    ...
    if ax_idx == 0:
        ...
        ax.annotate('', xy=(0, 1.4), xytext=(0, 0.6),
                   arrowprops=dict(arrowstyle='->', color='#888', linewidth=1.5))
        ax.text(1.0, -0.35, '向右', ha='center', fontsize=7, color='#888')
        ax.text(-0.35, 1.0, '向下', ha='center', fontsize=7, color='#888', rotation=90)

    # Panel (b): draw backward fill arrows
    if ax_idx == 1:
        # Show a few backward arrows to indicate fill direction
        ax.annotate('逆向\n填充', xy=(3.5, 3.5), fontsize=8, color='#C62828',
