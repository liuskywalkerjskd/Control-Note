#!/usr/bin/env python3
"""
Generate EKF 6-DOF IMU pose estimation figure.
...
def fig_ekf_imu():
    ...
    axes[0,0].plot(t, accel_roll*deg, 'C3', alpha=0.15, linewidth=0.5, label='Accel only')
    axes[0,0].plot(t, gyro_roll*deg, 'C1', alpha=0.6, linewidth=1, label='Gyro integration')
    axes[0,0].plot(t, ekf_roll*deg, 'C0', linewidth=2, label='EKF')
    axes[0,0].plot(t, roll_true*deg, 'k--', linewidth=1.5, label='True')
    axes[0,0].set_ylabel('Angle (°)'); axes[0,0].set_title('Roll')
    axes[0,0].legend(fontsize=7, loc='upper right')

    axes[0,1].plot(t, accel_pitch*deg, 'C3', alpha=0.15, linewidth=0.5, label='Accel only')
    axes[0,1].plot(t, gyro_pitch*deg, 'C1', alpha=0.6, linewidth=1, label='Gyro integration')
    axes[0,1].plot(t, ekf_pitch*deg, 'C0', linewidth=2, label='EKF')
    axes[0,1].plot(t, pitch_true*deg, 'k--', linewidth=1.5, label='True')
    axes[0,1].set_title('Pitch'); axes[0,1].legend(fontsize=7, loc='upper right')

    axes[0,2].plot(t, gyro_yaw*deg, 'C1', alpha=0.6, linewidth=1, label='Gyro integration')
    axes[0,2].plot(t, ekf_yaw*deg, 'C0', linewidth=2, label='EKF')
    axes[0,2].plot(t, yaw_true*deg, 'k--', linewidth=1.5, label='True')
    axes[0,2].set_title('Yaw (no accel correction!)'); axes[0,2].legend(fontsize=7)

    for ax_idx, (label, true_val) in enumerate(zip(
        ['Bias X', 'Bias Y', 'Bias Z'], gyro_bias_true)):
        axes[1, ax_idx].plot(t, ekf_bias[ax_idx]*deg, 'C0', linewidth=2, label='EKF estimate')
        axes[1, ax_idx].axhline(true_val*deg, color='k', linestyle='--',
                               linewidth=1.5, label=f'True = {true_val*deg:.2f} °/s')
        axes[1, ax_idx].set_xlabel('Time (s)'); axes[1, ax_idx].set_ylabel('Bias (°/s)')
        axes[1, ax_idx].set_title(f'Gyro {label} Estimation')
        axes[1, ax_idx].legend(fontsize=8)

    for ax in axes[0, :]:
        ax.axvspan(5, 7, alpha=0.1, color='C3')
        ax.text(6, ax.get_ylim()[1]*0.85, 'vibration', ha='center',
               fontsize=7, color='C3', fontstyle='italic')

    fig.suptitle(
        f'EKF 6-DOF IMU (auto-tuned) — RMSE: roll={result["rmse_roll"]:.2f}°, '
        f'pitch={result["rmse_pitch"]:.2f}°, yaw={result["rmse_yaw"]:.2f}°',
        fontsize=12, y=1.01)
    plt.tight_layout()
